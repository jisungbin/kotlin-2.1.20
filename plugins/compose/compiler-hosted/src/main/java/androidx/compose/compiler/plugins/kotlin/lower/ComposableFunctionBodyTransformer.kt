...
class ComposableFunctionBodyTransformer(
  ...
  private fun visitRestartableComposableFunction(
    ...
    val end = {
      irEndRestartGroupAndUpdateScope(
        ...
        numRealValueParameters = scope.realValueParamCount
      )
    }
    ...
    // conditions are met.
    //
    // 선택적이지 않은 불안정한 파라미터가 있다면, 해당 함수는 절대 스킵할 수 없으므로
    ...
    val transformedBody = if (canSkipExecution) {
      ...
      //
      // 강력한 스킵 모드가 아니고 불안정한 파라미터가 있는 경우, 전체 표현식을 감싸서
      // 해당 파라미터 중 실제로 함수에 전달된 것이 있는지 확인합니다. 만약 전달되었다면,
      // 조건 검사를 생략하고 본문을 항상 실행합니다.
      if (
        !FeatureFlag.StrongSkipping.enabled &&
        hasAnyUnstableParams && defaultParam != null
      ) {
        shouldExecute = irOrOr(
          lhs = defaultParam.irHasAnyProvidedAndUnstable(unstableMask),
          rhs = shouldExecute,
        )
      }
      ...
      irComposite(statements = bodyPreamble.statements + transformed.statements)
    }
  ...
  private fun buildPreambleStatementsAndReturnIsSkippable(
    ...
    trackedParameters.fastForEachIndexed { slotIndex, param ->
      ...
      if (mightSkip && dirtyBitMaskValue is IrChangedBitMaskVariable) {
        ...
        val irGetParamSize = irMethodCall(target = irGet(param), function = sizeGetter)

        statements.add(
          irStartMovableGroup(
            element = param,
            joinedData = irGetParamSize,
            scope = defaultScope,
          ),
        )
        ...
        statements.add(
          dirtyBitMaskValue.irOrSetBitsAtSlot(
            ...
            value = irIfThenElse(
              type = context.irBuiltIns.intType,
              condition = irChanged(
                value = irMethodCall(target = irGet(param), function = sizeGetter),
    ...
    //
    // 이 모든 작업 이후에는, 기본값 설정 코드를 그룹과 if 문으로 감쌀 필요가 있습니다.
    // 이를 통해 기본값이 실제로 필요한 경우에만 실행되도록 보장합니다.
  ...
  private fun irCallChanged(
    ...
    if (FeatureFlag.StrongSkipping.enabled && stability.isUncertain()) {
      irIfThenElse(
        ...
        condition = irIsStable(changed = changedParam, slot = slotIndex),
        thenPart = irChanged(
  ...
  private fun irEndRestartGroupAndUpdateScope(
    ...
    val defaultIndex = changedIndex + changedParamCount(
      realValueParamsCount = numRealValueParameters,
      thisParamsCount = function.thisParamCount,
    )
      ...
      fn.body = DeclarationIrBuilder(context, fn.symbol).irBlockBody {
        ...
        +irReturn(
          irCall(function.symbol).apply {
            ...
            defaultParam?.putAsValueArgumentIn(fn = this, startIndex = defaultIndex)
    ...
    return irBlock(
      statements = listOfNotNull(
        outerReceiver,
        irSafeCall(
          target = irEndRestartGroup(scope = scope),
          symbol = updateScopeFunction.symbol,
          lambda, // arguments
        ),
      )
    )
  ...
  private fun irIsUncertainAndStable(changed: IrChangedBitMaskValue, slot: Int): IrExpression =
    irEqual(
      lhs = changed.irIsolateBitsAtSlot(slot, includeStableBit = true),
      rhs = irIntConst(0),
    )

  private fun irIsStable(changed: IrChangedBitMaskValue, slot: Int): IrExpression =
    irEqual(
      lhs = changed.irStableBitAtSlot(slot),
      rhs = irIntConst(0),
    )

  private fun irIsUncertain(changed: IrChangedBitMaskValue, slot: Int): IrExpression =
    irEqual(
      lhs = changed.irIsolateBitsAtSlot(slot, includeStableBit = false),
      rhs = irIntConst(0)
    )

  @Suppress("SameParameterValue")
  private fun irBitsForSlot(bits: Int, slot: Int): IrExpression = irIntConst(bitsForSlot(bits, slot))
  ...
  private fun visitRememberCall(expression: IrCall): IrExpression {
    ...
    // different.
    //
    // $changed나 $dirty는 restart 함수에서 해당 플래그들이 정확히 갱신되는 경우에만 신뢰할
    // 수 있습니다. 그렇지 않으면 remember를 cached로 대체했을 때 결과가 달라질 수 있습니다.
  ...
  private fun irIntrinsicChanged(
    ...
    return when {
      ...
      argInfo.isCertain &&
        ...
        //
        // 디폴트 값이 없는 파라미터이고 안정적인 값이라는 것이 확실할 경우, dirty flag일지라도
  ...
  private fun buildChangedArgumentsForCall(
    ...
    val changedCount = changedParamCount(realValueParamsCount = allArgs.size, thisParamsCount = 0)
  ...
  private fun buildChangedArgumentForCall(arguments: List<CallArgumentMeta>): IrExpression {
    ...
    arguments.fastForEachIndexed { slot, argInfo ->
      ...
      when {
        !FeatureFlag.StrongSkipping.enabled && stability.knownUnstable() -> {
          ...
          //
          // 알려진 바와 같이 안정적이지 않은 경우, 이 매개변수에 대해서는 추가 메타데이터를
    ...
    return when {
      ...
      //
      // 상수가 여전히 0이라면 or 표현식만 사용해도 됩니다. 이는 or 표현식의 결과와 관계없이
            ...
            if (maskParam != null && fn.typeParameters.isNotEmpty()) {
              for (it in fn.valueParameters) {
                ...
                if (classifier == param.symbol) {
                  ...
                  return irAnd(
                    lhs = irIntConst(StabilityBits.UNSTABLE.bitsForSlot(slot = 0)),
                    rhs = maskParam.irShiftBits(fromSlot = parentSlot, toSlot = 0),
                  )
                }
              }
            }
  ...
  sealed class Scope(val name: String) {
    ...
    class FunctionScope(
      ...
      val allTrackedParams: List<IrValueParameter> =
        buildList {
    ...
    override fun irHasAnyProvidedAndUnstable(unstable: BooleanArray): IrExpression {
      ...
      val expressions = defaultParams.mapIndexed { index, param ->
        ...
        irNotEqual(
          ...
          lhs = irAnd(lhs = irGet(param), rhs = irIntConst(unstableMask)),
          rhs = irIntConst(unstableMask),
        )
      }
      return if (expressions.size == 1) expressions.single() else expressions.reduce { lhs, rhs -> irOrOr(lhs, rhs) }
    }
  ...
  open inner class IrChangedBitMaskValueImpl(
    ...
    override fun irHasDifferences(usedParams: BooleanArray): IrExpression {
      ...
      val expressions = changedParams.mapIndexed { index, param ->
        ...
        if (rhs == 0) {
          irNotEqual(
            lhs = irAnd(lhs = irGet(param), rhs = irIntConst(1)),
            rhs = irIntConst(0)
          )
        } else {
          // $dirty and (0b 101 ... 101 1) != (0b 001 ... 001 0)
          irNotEqual(
            lhs = irAnd(lhs = irGet(param), rhs = irIntConst(lhs or 0b1)),
            rhs = irIntConst(rhs or 0b0),
          )
        }
      }
    ...
    private fun irUpdateChangedFlags(expression: IrExpression): IrExpression {
      return updateChangedFlagsFunction?.let { simpleFunction ->
        irCall(simpleFunction).also { call -> call.putValueArgument(0, expression) }
      } ?: expression
    }
    ...
    override fun irShiftBits(fromSlot: Int, toSlot: Int): IrExpression {
      ...
      val shiftRight = int.binaryOperator(
        name = OperatorNameConventions.SHR,
        paramType = int,
      )
