...
internal fun <T, R> Iterable<T>.maybeChunked(size: Int?, transform: (List<T>) -> R): List<R> = size?.let { this.chunked(size, transform) } ?: listOf(transform(this.toList()))
...
abstract class KlibMetadataSerializer(
  ...
  ) = topSerializer.packagePartProto(fqName, descriptors).build()
  ...
  protected abstract val TOP_LEVEL_DECLARATION_COUNT_PER_FILE: Int?
  protected abstract val TOP_LEVEL_CLASS_DECLARATION_COUNT_PER_FILE: Int?
}
