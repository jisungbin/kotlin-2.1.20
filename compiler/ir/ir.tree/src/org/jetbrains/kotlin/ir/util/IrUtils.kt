...
fun IrConstImpl.Companion.defaultValueForType(startOffset: Int, endOffset: Int, type: IrType): IrConstImpl {
  ...
  return when (type.getPrimitiveType()) {
    PrimitiveType.BOOLEAN -> IrConstImpl.boolean(startOffset, endOffset, type = type, value = false)
    PrimitiveType.CHAR -> IrConstImpl.char(startOffset, endOffset, type = type, value = '0')
    PrimitiveType.BYTE -> IrConstImpl.byte(startOffset, endOffset, type = type, value = 0)
    PrimitiveType.SHORT -> IrConstImpl.short(startOffset, endOffset, type = type, value = 0)
    PrimitiveType.INT -> IrConstImpl.int(startOffset, endOffset, type = type, value = 0)
    PrimitiveType.FLOAT -> IrConstImpl.float(startOffset, endOffset, type = type, value = 0.0F)
    PrimitiveType.LONG -> IrConstImpl.long(startOffset, endOffset, type = type, value = 0)
    PrimitiveType.DOUBLE -> IrConstImpl.double(startOffset, endOffset, type = type, value = 0.0)
    else -> IrConstImpl.constNull(startOffset, endOffset, type = type.makeNullable())
  }
}
